 ...
 */
class MenuDigital extends BaseController
{
    protected $modelName = 'App\Models\Pulsa\MenuDigitalModel';
    ...
    protected $rules = [
        ...
        'jenisMenu' => ['label' => 'jenisMenu', 'rules' => 'required'],
        'showHome' => ['label' => 'showHome', 'rules' => 'required'],
        'enabled' => ['label' => 'enabled', 'rules' => 'required'],
    ...
    {
        $data = [
            'selectJenisMenu' => $this->model->selectJenisMenu(),
        ];

        return $this->template->setActiveUrl('MenuDigital')
            ->view("MenuDigital/index", $data);
    }
    ...
    protected function uploadFile($id)
    {
        ...
        $path = Config::get("App")->uploadPath . PATH_ICON_MENU_DIGITAL;
        if (!is_dir($path)) {
    ...
    {
        $this->model->orderBy('mdUrutan', 'ASC');
